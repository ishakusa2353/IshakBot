(function(){
  ...
  // ⚡ 6.5. QUOTEX AUTO-TRADE BULLETPROOF NATIVE CLICK DISPATCHER
  function dispatchRealQuotexClick(el) {
    if (!el) return false;
    try {
    ...
    var cx = rect.left + (rect.width > 0 ? rect.width / 2 : 15);
    var cy = rect.top + (rect.height > 0 ? rect.height / 2 : 15);
    ...
    // Full Pointer + Mouse lifecycle required by React 17/18 fiber event system
    try { el.dispatchEvent(new PointerEvent('pointerover', evtInit)); } catch(e){}
    try { el.dispatchEvent(new PointerEvent('pointerenter', evtInit)); } catch(e){}
    ...
    try { el.dispatchEvent(new PointerEvent('pointerup', evtInit)); } catch(e){}
    ...
    // Click child element (e.g. inner span / svg / div)
    try {
      var child = el.querySelector('span, svg, div, p, b');
  ...
  // 🎯 FIND QUOTEX BUTTON WITH MULTI-LAYER RECOGNITION
  function findQuotexTradeButton(isCall) {
    // 1. Direct CSS Selectors (All known Quotex Desktop & Mobile platforms)
    var selectors = isCall ? [
    ...
    // 2. Deal Section Traverse (Matches Text & Classes in all Quotex languages)
    var dealAreas = document.querySelectorAll('.section-deal, .deal-form, .panel-deal, [class*="deal"], aside, #platform-container');
    for (var d = 0; d < dealAreas.length; d++) {
      var allClickables = dealAreas[d].querySelectorAll('button, .button, div[role="button"], a[role="button"], [class*="button"]');
      for (var c = 0; c < allClickables.length; c++) {
        var btn = allClickables[c];
        if (btn.closest('#ishak-trade-wrap') || btn.closest('#ishak-hud-panel') || btn.closest('.ishak-dialog-modal')) continue;
        ...
        if (isCall) {
          if (
            txt === 'UP' || txt === 'CALL' || txt === 'HIGHER' || txt === 'BUY' ||
            txt.indexOf('ВВЕРХ') !== -1 || txt.indexOf('ВЫШЕ') !== -1 ||
            txt.indexOf('SUBIR') !== -1 || txt.indexOf('ACIMA') !== -1 ||
            txt.indexOf('ARRIBA') !== -1 || txt.indexOf('উপরে') !== -1 ||
            cls.indexOf('call') !== -1 || cls.indexOf('--up') !== -1 ||
            cls.indexOf('success') !== -1 || cls.indexOf('green') !== -1
          ) {
            return btn;
          }
        } else {
          if (
            txt === 'DOWN' || txt === 'PUT' || txt === 'LOWER' || txt === 'SELL' ||
            txt.indexOf('ВНИЗ') !== -1 || txt.indexOf('НИЖЕ') !== -1 ||
            txt.indexOf('BAJAR') !== -1 || txt.indexOf('ABAIXO') !== -1 ||
            txt.indexOf('ABAJO') !== -1 || txt.indexOf('নিচে') !== -1 ||
            cls.indexOf('put') !== -1 || cls.indexOf('--down') !== -1 ||
            cls.indexOf('danger') !== -1 || cls.indexOf('red') !== -1
          ) {
            return btn;
          }
        }
    ...
    // 3. Computed Color Analysis (Detects green or red button in DOM)
    var allPageButtons = document.querySelectorAll('button, div[role="button"], .button');
    for (var p = 0; p < allPageButtons.length; p++) {
      var candidate = allPageButtons[p];
      if (candidate.closest('#ishak-trade-wrap') || candidate.closest('#ishak-hud-panel') || candidate.closest('.ishak-dialog-modal')) continue;

      var bRect = candidate.getBoundingClientRect();
      if (bRect.width >= 40 && bRect.height >= 25 && bRect.top >= 0 && bRect.top <= window.innerHeight) {
        var style = window.getComputedStyle(candidate);
        var bg = style.backgroundColor || '';
        var mColor = bg.match(/rgba?\((\d+),\s*(\d+),\s*(\d+)/);
        if (mColor) {
          var r = parseInt(mColor[1], 10);
          var g = parseInt(mColor[2], 10);
          var b = parseInt(mColor[3], 10);

          if (isCall && g > r + 30 && g > b + 25) {
            return candidate;
          }
          if (!isCall && r > g + 40 && r > b + 40) {
            return candidate;
          }
        }
      }
  ...
  // 🚀 QUOTEX AUTO-TRADE EXECUTION ENGINE
  ...
  function executeQuotexTrade(isCall, signalId) {
    ...
    var now = Date.now();
    if (now - lastTradeTimestamp < 2500) {
    ...
    try {
      var targetBtn = findQuotexTradeButton(isCall);
      ...
      if (targetBtn) {
        ...
        dispatchRealQuotexClick(targetBtn);

        setTimeout(function() {
          dispatchRealQuotexClick(targetBtn);
        }, 40);

        return { success: true, element: targetBtn };
      } else {
